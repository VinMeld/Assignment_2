String[] sentence = loadStrings("othello.txt");
...
void setup() {
  background(0);
  loadStrings("illiad.txt");
  loadStrings("romeoAndJuliet.txt");
  loadStrings("theOdyssey.txt");
  loadStrings("hamlet.txt");
  loadStrings("macbeth.txt");
  size(600, 600);
  lengthOfSentence();
  capitalLetters();
  vowels();
